...
Public Class RPT_GD_Present_Lotvaliable_HH_By_Floor_All_Day_Distinct_frm
 ...
 Optional ByRef DateEnd As String = "")
    ...
    Sub Query_Report_ByFloor()
        ...
        DateSt = cmb_Month.Text
        DateEnd = cmb_Year.Text

        'Str_Quy = "Select * from GD_Present_Carcount_By_Floor_All_Day "
        'Str_Quy = "Select * from GD_Present_Lotvaliable_HH_By_Floor_All_Day_Distinct "
        Str_Quy = "Select * from Tbl_Report_GD_Present_Lotvaliable_HH_By_Floor_All_Day "
        'GD_Present_Parking_OUT_HH_By_Floor_All_day_Distinct
        ...
        If cmb_Tower.SelectedIndex > 0 Then
            Str_Quy &= " where  Trn_Tower_ID = '" & cmb_Tower.SelectedValue & "'"
            If cmb_Building.SelectedIndex > 0 Then
                Str_Quy &= " and  Trn_Building_ID = '" & cmb_Building.SelectedValue & "'"
            End If
            If cmb_Floor.SelectedIndex > 0 Then
                Str_Quy &= " and  Trn_Floor_No = " & cmb_Floor.SelectedValue & ""
                'Str_Quy &= " and  Floor_Name = '" & cmb_Floor.Text & "'"
            End If
            If cmb_Month.SelectedIndex > 0 Then
                Str_Quy &= " and  Trn_Month_Name = '" & cmb_Month.Text & "'"
            End If

            If cmb_Year.SelectedIndex > 0 Then
                Str_Quy &= " and  Trn_Year = '" & cmb_Year.Text & "'"
            End If
        ElseIf cmb_Building.SelectedIndex > 0 Then
            Str_Quy &= " where  Trn_Building_ID = '" & cmb_Building.SelectedValue & "'"
            If cmb_Floor.SelectedIndex > 0 Then
                Str_Quy &= " and  Trn_Floor_No = " & cmb_Floor.SelectedValue & ""
                'Str_Quy &= " and  Floor_Name = '" & cmb_Floor.Text & "'"
            End If
        ...
        ElseIf cmb_Month.SelectedIndex > 0 Then
            Str_Quy &= " where  Trn_Month_Name = '" & cmb_Month.Text & "'"
            If cmb_Year.SelectedIndex > 0 Then
                Str_Quy &= " and  Trn_Year = '" & cmb_Year.Text & "'"
            End If
        ElseIf cmb_Year.SelectedIndex > 0 Then
            Str_Quy &= " where  Trn_Year = '" & cmb_Year.Text & "'"
        End If
        Str_Quy &= " order by Tower_Name,Trn_Year,Trn_Month_Name,Building_Name,Trn_Floor_No"

        pDtb_Qury = ClsSqlCmd.ReturnDatatable_(ConnStrGuidance_RPT, Str_Quy)
        Dim RPT As Object
        RPT = New RPT_GD_Present_Lotvaliable_HH_By_Floor_All_Day_Distinct
    ...
    End Sub
    Private Sub RPT_CAR_PARKING_BY_DAY_ADD1_frm_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Dim Year_in_Digits = Year(Now)
        Dim Month_Name = DateTime.Now.ToString("MMMM")

        cmb_Floor.Enabled = False
        cmb_Month.Enabled = True
        cmb_Month.Text = Month_Name
        cmb_Year.Enabled = True
        cmb_Year.Text = Year_in_Digits
