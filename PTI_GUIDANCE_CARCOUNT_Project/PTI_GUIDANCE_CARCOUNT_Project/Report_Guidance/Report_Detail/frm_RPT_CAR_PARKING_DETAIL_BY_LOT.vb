...
Public Class frm_RPT_CAR_PARKING_DETAIL_BY_LOT
    Dim ClsSqlCmd As New ClassCommandSql
    Friend pHeader_Detail As String
    Friend pHW_ID As String
...
Optional ByRef DateEnd As String = "")
    ...
    Sub Query_Report()
        ...
        Str_Quy = "Select * from VQR_CAR_PARKING_DETAIL_BY_LOT "
        ...
        If optIn.Checked = True Then Str_Quy &= " and Trn_Process='IN'"
        If optOut.Checked = True Then Str_Quy &= " and Trn_Process='OUT'"
        If pHW_ID <> "" Then Str_Quy &= " and   Trn_Lot_Id='" & pHW_ID & "'"
        Str_Quy &= " order by  [Trn_Tower_ID],[Trn_Building_ID],[Trn_Floor_No],[Trn_Month],[Trn_Log_Datetime_In],[Trn_Lot_Id]  asc"

        pDtb_Qury = ClsSqlCmd.ReturnDatatable_(ConnStrGuidance_RPT, Str_Quy)
        Dim RPT As Object
        RPT = New RPT_CAR_PARKING_DETAIL_BY_LOT
        ShowReport(pDtb_Qury, pHeader_Detail, RPT, DateSt, DateEnd)

    End Sub

    Private Sub frm_RPT_CAR_PARKING_DETAIL_BY_LOT_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
