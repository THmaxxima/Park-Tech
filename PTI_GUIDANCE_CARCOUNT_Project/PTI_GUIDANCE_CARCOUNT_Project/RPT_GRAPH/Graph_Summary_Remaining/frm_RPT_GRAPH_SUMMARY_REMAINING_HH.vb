...
Public Class frm_RPT_GRAPH_SUMMARY_REMAINING_HH
 ...
 Optional ByRef DateEnd As String = "")
    ...
    Sub Query_Report()
        ...
        Str_Quy = "Select * from QR_GRAPH_CAR_IN_OUT_SUMMARY_REMAINING_BY_HH "
        Str_Quy &= " where (Trn_Time_HH_Start between " & mDB.DBFormatDate(DateSt) & " and " & mDB.DBFormatDate(DateEnd) & ")"
    ...
    Sub Query_Report_ByTower()
        ...
        Str_Quy = "Select * from QR_GRAPH_CAR_IN_OUT_SUMMARY_REMAINING_BY_HH_TOWER "
        Str_Quy &= " where (Trn_Time_HH_Start between " & mDB.DBFormatDate(DateSt) & " and " & mDB.DBFormatDate(DateEnd) & ")"
        ...
        Str_Quy &= " order by Trn_Tower_ID,Trn_Time_HH_Start"

        pDtb_Qury = ClsSqlCmd.ReturnDatatable_(ConnStrGuidance_RPT, Str_Quy)
        Dim RPT As Object
        If rdo_Bar.Checked = True Then
            RPT = New BAR_RPT_GRAPH_SUMMARY_REMAINING_HH_TOWER
            ShowReport(pDtb_Qury, pHeader_Detail, RPT, DateSt, DateEnd)
        End If
        If rdo_Line.Checked = True Then
            RPT = New LINE_RPT_GRAPH_SUMMARY_REMAINING_HH_TOWER
            ShowReport(pDtb_Qury, pHeader_Detail, RPT, DateSt, DateEnd)
        End If
    ...
    Sub Query_Report_Floor()
        ...
        Str_Quy = "Select * from QR_GRAPH_CAR_IN_OUT_SUMMARY_REMAINING_BY_HH_FLOOR "
        Str_Quy &= " where (Trn_Time_HH_Start between " & mDB.DBFormatDate(DateSt) & " and " & mDB.DBFormatDate(DateEnd) & ")"
        ...
        Str_Quy &= " order by Trn_Tower_ID,Trn_Building_ID,Trn_Floor_No,Trn_Time_HH_Start"

        pDtb_Qury = ClsSqlCmd.ReturnDatatable_(ConnStrGuidance_RPT, Str_Quy)
        Dim RPT As Object
        If rdo_Bar.Checked = True Then
            RPT = New BAR_RPT_GRAPH_SUMMARY_REMAINING_HH_FLOOR
            ShowReport(pDtb_Qury, pHeader_Detail, RPT, DateSt, DateEnd)
        End If
        If rdo_Line.Checked = True Then
            RPT = New LINE_RPT_GRAPH_SUMMARY_REMAINING_HH_FLOOR
            ShowReport(pDtb_Qury, pHeader_Detail, RPT, DateSt, DateEnd)
        End If

    End Sub
    Private Sub frm_RPT_GRAPH_SUMMARY_REMAINING_HH_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
