...
Public Class frmRPT_GRAPH_CAR_PARKING_BY_15MM
 ...
 Optional ByRef DateEnd As String = "")
    ...
    Sub Query_Report()
        ...
        Str_Quy = " Select * from QVR_CAR_PARKING_15MM_BY_BUILDING "
        Str_Quy &= " where (Trn_Time_MM_Start15 between " & mDB.DBFormatDate(DateSt) & " and " & mDB.DBFormatDate(DateEnd) & ")"
    ...
    Sub Query_Report_ByTower()
        ...
        Str_Quy = "Select * from QVR_CAR_PARKING_15MM_BY_TOWER "
        Str_Quy &= " where (Trn_Time_MM_Start15 between " & mDB.DBFormatDate(DateSt) & " and " & mDB.DBFormatDate(DateEnd) & ")"
        ...
        Str_Quy &= " order by Trn_Tower_ID,Trn_Time_MM_Start15 "

        pDtb_Qury = ClsSqlCmd.ReturnDatatable_(ConnStrGuidance_RPT, Str_Quy)
        Dim RPT As Object
        If rdo_Bar.Checked = True Then
            RPT = New BAR_RPT_GRAPH_CAR_PARKING_BY_15MM_TOWER
            ShowReport(pDtb_Qury, pHeader_Detail, RPT, DateSt, DateEnd)
        End If
        If rdo_Line.Checked = True Then
            RPT = New LINE_RPT_GRAPH_CAR_PARKING_BY_15MM_TOWER
            ShowReport(pDtb_Qury, pHeader_Detail, RPT, DateSt, DateEnd)
        End If


    End Sub
    Sub Query_Report_Floor()
        Dim Str_Quy As String = ""
        Dim pDtb_Qury As DataTable
        Dim DateSt As DateTime
        Dim DateEnd As DateTime
        ...
        Str_Quy = "Select * from QVR_CAR_PARKING_15MM_BY_FLOOR "
        Str_Quy &= " where (Trn_Time_MM_Start15 between " & mDB.DBFormatDate(DateSt) & " and " & mDB.DBFormatDate(DateEnd) & ")"
        ...
        Str_Quy &= " order by Trn_Tower_ID,Trn_Building_ID,Trn_Floor_No,Trn_Time_MM_Start15 "

        pDtb_Qury = ClsSqlCmd.ReturnDatatable_(ConnStrGuidance_RPT, Str_Quy)
        Dim RPT As Object
        If rdo_Bar.Checked = True Then
            RPT = New BAR_RPT_GRAPH_CAR_PARKING_BY_15MM_FLOOR
            ShowReport(pDtb_Qury, pHeader_Detail, RPT, DateSt, DateEnd)
        End If
        If rdo_Line.Checked = True Then
            RPT = New LINE_RPT_GRAPH_CAR_PARKING_BY_15MM_FLOOR
            ShowReport(pDtb_Qury, pHeader_Detail, RPT, DateSt, DateEnd)
        End If


    End Sub
    Private Sub frmRPT_GRAPH_CAR_PARKING_BY_15MM_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub cmb_Building_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmb_Building.SelectedIndexChanged
        ...
        If cmb_Building.SelectedIndex <= 0 Then Exit Sub
        AddCombo2(ConnStrMasTer, Me.cmb_Floor, "Mas_Floor", "FUll_Name", "Floor_ID", " Building_ID = " & cmb_Building.SelectedValue & "", "Building_ID,Floor_ID", "<--เลือกทั้งหมด-->", "Floor_ID ,  convert(nvarchar(255),Building_ID) + '  ' +  convert(nvarchar(255),Floor_ID) +  ' : ' +  Floor_Name  as FUll_Name")
        cmb_Floor.Enabled = True
    ...
    Private Sub btn_Show_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Show.Click
        ...
        If cmb_Tower.SelectedIndex <= 0 And cmb_Building.SelectedIndex <= 0 Then
        ...
        If cmb_Floor.SelectedIndex > 0 Then
            Call Query_Report_Floor()
        Else
            If cmb_Building.SelectedIndex > 0 Then
                Query_Report()
            Else
                Query_Report() ' Call Query_Report_ByTower()
            End If
        End If
        btn_Show.Enabled = True
    End Sub
